...
class DeviceCodes {
    ...
    private function api_token_check() {
        ...
        $token_expiration = (int) get_option("dotstudiopro_api_token_time");
        ...
        if (!empty($token_expiration) && is_numeric($token_expiration) && $token_expiration <= time() || ( empty($token_expiration) || empty($token) || !is_numeric($token_expiration) || $token_expiration > $future )) {
            $api_key = get_option("dotstudiopro_api_key");
            ...
            if ($result->err) {
                ...
                if ($res->success) {
                    ...
                    update_option("dotstudiopro_api_token_time", time() + (3600 * 24 * 29));
                }
            }
        }
        return $token;
